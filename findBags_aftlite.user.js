...
(function() {
    ...
    const findMissingBags = async (TRACKING_NUMBER) => {
        ...
        const findPicker = (DOCUMENT, TRACKING_NUMBER) => {
            ...
            const DOM = {};
            DOM.picklists = [...DOCUMENT.querySelector("#picklists_table").children[0].children];
            DOM.picklists.shift();
            ...
            picker.knownBags = {};
            picker.knownBags.inSameOrderAsMissingBag = orders.filter(order => order.user.name == picker.name && order.tracking !== picker.missingBag.tracking);
        ...
        const getBagsFromPicker = async (MATCHED_USER) => {
            ...
            const table = [...page.querySelector("table.reportLayout").childNodes[1].children];
            table.shift();
        ...
        const relatedBags = await Promise.all(pickerSpoosWithinHour.map(async (spoo) => convertSpooToTracking(spoo)));
        const relatedBagsFiltered = relatedBags.map(bag => bag.tracking).filter(tracking => tracking !== undefined);
        ...
        prompt("COPY THEN GO TO COMO & PRESS ( SHIFT + F ) THEN PASTE & PRESS ENTER", JSON.stringify(picker));
